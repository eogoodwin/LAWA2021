...
for(i in 1:length(sites)){
  ...
  for(j in 1:length(Measurements)){
    url <- paste0("http://tsdata.horizons.govt.nz/boo.hts?service=SOS&agency=LAWA&request=GetObservation",
                  ...
                  "&TemporalFilter=om:phenomenonTime,2005-01-01,2021-06-01")
    # url <- gsub(" ", "%20", url)
    url <- URLencode(url)
...
save(hrcM,file = 'hrcMraw.rData')
#load('hrcMraw.rData')

hrcM <-hrcM%>%dplyr::transmute(CouncilSiteID=Site,Date=time,Value=value,Measurement,
                                 CollMeth=NA,ProcMeth=NA)



hrcMb=data.frame(CouncilSiteID=hrcM$CouncilSiteID,
                  Date=as.character(format(lubridate::ymd_hms(hrcM$Date),'%d-%b-%y')),
                  Value=hrcM$Value,
                  Measurement=hrcM$Measurement,
                  Censored=grepl(pattern = '<|>',x =hrcM$Value),
                  CenType=F,
                  CollMeth=hrcM$CollMeth,
                  ProcMeth=hrcM$ProcMeth)
hrcMb$CenType[grep('<',hrcMb$Value)] <- 'Left'
hrcMb$CenType[grep('>',hrcMb$Value)] <- 'Right'

hrcMb$Value = readr::parse_number(hrcMb$Value)
...
hrcMb$measName=hrcMb$Measurement
table(hrcMb$Measurement,useNA = 'a')
hrcMb$Measurement <- as.character(factor(hrcMb$Measurement,
                                          levels = Measurements,
                                          labels = c("MCI","PercentageEPTTaxa",'TaxaRichness',"QMCI","ASPM")))
table(hrcMb$Measurement,hrcMb$measName,useNA='a')


#hrcMb <- merge(hrcMb,siteTable,by='CouncilSiteID')


write.csv(hrcMb,file = paste0("D:/LAWA/2021/hrcM.csv"),row.names = F)
file.copy(from=paste0("D:/LAWA/2021/hrcM.csv"),
          to = paste0("H:/ericg/16666LAWA/LAWA2021/MacroInvertebrates/Data/",format(Sys.Date(),"%Y-%m-%d"),"/hrc.csv"),
          overwrite = T)
# write.csv(hrcMb,file = paste0("H:/ericg/16666LAWA/LAWA2021/macroinvertebrates/Data/",format(Sys.Date(),"%Y-%m-%d"),"/",agency,".csv"),row.names = F)
rm(hrcM,hrcMb)
