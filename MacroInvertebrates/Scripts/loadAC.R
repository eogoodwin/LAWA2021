...
acMacros <- acmacros%>%
  dplyr::select(CouncilSiteID,LAWASiteID,SiteID,Date=SampleDate,
                MCI,SQMCI,ASPM=APSM,TotalRichness=TaxaRichness,
                "%EPTRichness","Quality Code")%>%
    tidyr::gather(key="Measurement",value="Value",c("MCI",SQMCI,ASPM,"TotalRichness","%EPTRichness"))
acMacros$Agency='ac'
...
acMacros <- acMacros%>%filter(Measurement!="SQMCI")
write.csv(acMacros,
