...
# •	Checking the numbers are correct for 2020 – some of the titles are incorrect so unsure we are accurately presenting the correct years etc. 
...
TLI <- merge(x=lakeTable%>%select(LawaSiteID,SiteID,CouncilSiteID,
                                  ...
                                  Region,Agency,Long,Lat,Elevation,DistToSea),all.x=F,
             y=TLI,all.y=T)
...
NOFSummaryTable <- merge(lakeTable%>%select(LawaSiteID,SiteID,CouncilSiteID,LFENZID,
                                            ...
                                            Long,Lat,Elevation,DistToSea),all.x=F,
...
write.csv(data.frame(barTLI)%>%dplyr::rename("all125"=X1,"lowland79"=X2,"upland46"=X3),
...
axis(side = 1,at=iT,labels = c(expression(atop(All~elevations,'(125 sites)')),
                               expression(atop(Lowland*' < 170m','(79 sites)')),
                               expression(atop(Upland*" > 170m",'(46 sites)'))),
...
axis(side=1,at=c(0.3,0.8),c(expression(atop(Lowland*' < 170m','(79 sites)')),
                            expression(atop(Upland*" > 170m",'(46 sites)'))),lty=0,cex.axis =6)
par(mgp=c(4,1.5,0))
