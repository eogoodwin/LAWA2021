...
riverWQStateW <- merge(x=riverSiteTable%>%filter(Agency!='niwa')%>%
                         dplyr::select(Region,Agency,LawaSiteID,SiteID,CouncilSiteID,
                                       Lat,Long,SWQLanduse,SWQAltitude,recLandcover,Catchment),all.x=F,
...
riverNOF$Indicator[riverNOF$Indicator=="SusSed"] <- "CLAR"
...
NOFTableOut = NOFSummaryTable%>%select(LawaSiteID:Year,NZReach:Long,
                                       ...
                                       DIN_Summary_Band,NO3N_Toxicity_Band,Ammonia_Toxicity_Band,
                                       EcoliSummaryband,DRP_Summary_Band,ClarityBand=SusSedBand,Nitrate_Toxicity_Band)
