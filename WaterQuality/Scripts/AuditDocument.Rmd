...
MCINOF <- read.csv(tail(dir(path = 'h:/ericg/16666LAWA/LAWA2020/MacroInvertebrates/Analysis/',
...
MCINOF$NOF2020Band = cut(MCINOF$rollMCI,breaks = c(0,90,110,130,200),right = F,labels = c("D","C","B","A"))
...
nfts <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/WaterQuality/Analysis",
                          ...
                          full.names=T,recursive=T,ignore.case=T),1),stringsAsFactors=F)
nfts <- nfts%>%filter(LawaSiteID%in%agencyLIDs) #filter(Year=="2015to2019")%>%
...
```{r, echo=F,results='asis'}
DT::datatable(MCINOF%>%dplyr::select(LawaSiteID,CouncilSiteID,yearTo=sYear,`5yrRollMCI`=rollMCI,NOF2020Band),
