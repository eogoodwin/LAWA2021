...
#// StateScore <- function(df,scope,altitude,Landcover,wqparam,comparison)
...
# Count results by Site and Measurement for each date
# Count less thans for each Site and Measurement
# Count each less than value for each site and Measurement combo
# Remove greater thans for each site and Measurement
# Review results
...
MeasurementList <- function(xmlmdata,requestType){
  if(requestType=="SOS"){
    ...
    #site.attr<-getNodeSet(doc=xmlmdata,path="//T/ancestor::Measurement/@CouncilSiteID")
    #site.list<-sapply(site.attr, as.character)
    ...
    for(i in 1:length(a)){
      ...
      s<-sapply(c,function(el) xmlGetAttr(el, "CouncilSiteID"))
      ...
      myPath<-paste("//Measurement[",i,"]/Data/E/Measurement[@Name='Method']",sep="")
      ...
      if(length(f)==0){
        ...
        myPath<-paste("//Measurement[",i,"]/Data/E/Measurement[@Name='Method']/../T",sep="")
  ...
  names(df) <- c("Date","CouncilSiteID","Value","Method")
  df$CouncilSiteID <- gsub("''","'",df$CouncilSiteID)   # replacing the double apostrophe's generated by parsing the XML with single '.
  df<-df[,c(2,1,3,4)]
...
StateAnalysis <- function(df,type,level){
  ...
  # level = LandcoverAltitude,Landcover,Altitude,None
  
  # Returns the medians and counts, grouped by type and level
  ...
  # ' // s = sitemedians
  dfs <- summaryBy(Value~CouncilSiteID+LawaSiteID+SiteID+Measurement,
                   id=~SWQLanduse+SWQAltitude+Region+Agency+Frequency,
                   data=df, 
                   FUN=c(quantile), probs=c(0.5), type=5, na.rm=TRUE, keep.name=TRUE)
  
  # ==============================================================
  # Test number of samples by Site/Measurement
  # ==============================================================
  # Added 8-Oct-2015
  #
  # Exclusion criteria
  ...
  # samples by group, instead of the median.
    ...
    dfs_count <- summaryBy(Value~CouncilSiteID+LawaSiteID+SiteID+Measurement,
                         id=~SWQLanduse+SWQAltitude+Region+Agency,#+Frequency,
                         data=df,
                         FUN=c(length))
  
  dfs$Count <- dfs_count$Value.length   #renamed as N in the state script
  rm(dfs_count)
  ...
  if(type=="Site"){
    if(level=="LandcoverAltitude"){
      s <- summaryBy(Value~LawaSiteID+Measurement,  
                     id=~SWQAltitude+SWQLanduse+Region+Agency+CouncilSiteID+SiteID,
                     data=dfs, 
                     FUN=c(quantile), type=5, na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~LawaSiteID+Measurement,  
                     id=~SWQAltitude+SWQLanduse+Region+Agency+CouncilSiteID+SiteID,
                     data=dfs, 
    ...
    if(level=="LandcoverAltitude"){
      s <- summaryBy(Value~Catchment+SWQAltitude+SWQLanduse+Region+Agency+Measurement,
                     # id=~CouncilSiteID+LawaSiteID+SiteID,
                     data=dfs,
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~Catchment+SWQAltitude+SWQLanduse+Region+Agency+Measurement,
                     id=~CouncilSiteID+LawaSiteID+SiteID,
                     data=dfs,
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s$LawaSiteID <- "All"
      s$SiteID <- "All"
    } else if(level=="Landcover"){
      s <- summaryBy(Value~Catchment+SWQLanduse+Region+Agency+Measurement,
                     #id=~CouncilSiteID+LawaSiteID+SiteID+SWQAltitude,
                     data=dfs,
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~Catchment+SWQLanduse+Region+Agency+Measurement,
                     #id=~CouncilSiteID+LawaSiteID+SiteID+SWQAltitude,
                     data=dfs,
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s <- summaryBy(Value~Catchment+SWQAltitude+Region+Agency+Measurement,
                     #id=~CouncilSiteID+LawaSiteID+SiteID+SWQLanduse,
                     data=dfs,
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~Catchment+SWQAltitude+Region+Agency+Measurement,
                     #id=~CouncilSiteID+LawaSiteID+SiteID+SWQLanduse,
                     data=dfs,
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s <- summaryBy(Value~Catchment+Region+Agency+Measurement,
                     #id=~CouncilSiteID+LawaSiteID+SiteID+SWQAltitude+SWQLanduse,
                     data=dfs,
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~Catchment+Region+Agency+Measurement,
                     #id=~CouncilSiteID+LawaSiteID+SiteID+SWQAltitude+SWQLanduse,
                     data=dfs,
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
    ...
    if(level=="LandcoverAltitude"){
      s <- summaryBy(Value~SWQAltitude+SWQLanduse+Region+Measurement,
                     # id=~CouncilSiteID+LawaSiteID+SiteID+Agency,
                     data=dfs, 
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~SWQAltitude+SWQLanduse+Region+Measurement,  
                     # id=~CouncilSiteID+LawaSiteID+SiteID+Agency,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s <- summaryBy(Value~SWQLanduse+Region+Measurement,
                     # id=~CouncilSiteID+LawaSiteID+SiteID+SWQAltitude+Agency,
                     data=dfs, 
                     FUN=c(quantile), type=5, na.rm=TRUE, keep.name=TRUE) 
      t <- summaryBy(Value~SWQLanduse+Region+Measurement,  
                     # id=~CouncilSiteID+LawaSiteID+SiteID+SWQAltitude+Agency,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)     
      s$N<-t$Value
      ...
      s <- summaryBy(Value~SWQAltitude+Region+Measurement,
                     # id=~CouncilSiteID+LawaSiteID+SiteID+SWQLanduse+Agency,
                     data=dfs, 
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~SWQAltitude+Region+Measurement,  
                     # id=~CouncilSiteID+LawaSiteID+SiteID+SWQLanduse+Agency,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)  
      s$N<-t$Value
      ...
      s <- summaryBy(Value~Region+Measurement,
                     # id=~CouncilSiteID+LawaSiteID+SiteID+SWQLanduse+Agency,
                     data=dfs, 
                     FUN=c(quantile), type=5, na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~Region+Measurement,  
                     # id=~CouncilSiteID+LawaSiteID+SiteID+SWQLanduse+Agency,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
    ...
    if(level=="LandcoverAltitude"){
      s <- summaryBy(Value~SWQAltitude+SWQLanduse+Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID,
                     data=dfs, 
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~SWQAltitude+SWQLanduse+Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s <- summaryBy(Value~SWQLanduse+Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID+SWQAltitude,
                     data=dfs, 
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~SWQLanduse+Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID+SWQAltitude,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s <- summaryBy(Value~SWQAltitude+Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID+SWQLanduse,
                     data=dfs, 
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~SWQAltitude+Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID+SWQLanduse,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
      ...
      s <- summaryBy(Value~Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID+SWQLanduse+SWQAltitude,
                     data=dfs, 
                     FUN=c(quantile),na.rm=TRUE, keep.name=TRUE)
      t <- summaryBy(Value~Measurement,  
                     # id=~Region+Agency+CouncilSiteID+LawaSiteID+SiteID+SWQLanduse+SWQAltitude,
                     data=dfs, 
                     FUN=c(length), keep.name=TRUE)
      s$N<-t$Value
...
StateScore <- function(df,scopeIn,altitude,LandcoverIn,wqparam,comparison){
  ...
  #  comparision = 1,2,3,4
  #              1 = All -                 SWQAltitude=="All"    & SWQLanduse=="All"   & Scope=="NZ"
  #              2 = Altitude -            SWQAltitude==altitude & SWQLanduse=="All"   & Scope=="NZ"
  #              3 = Land use -            SWQAltitude=="All"    & SWQLanduse==LandcoverIn & Scope=="NZ"
  #              4 = Altitude & Land use - SWQAltitude==altitude & SWQLanduse==LandcoverIn & Scope=="NZ"
  ...
  for(i in 1:length(wqparam)){
    ...
    if(comparison==1){
      ...
      df_scope           <- subset(df, tolower(Scope)==tolower(scopeIn) &
                                     tolower(Measurement)==tolower(wqparam[i]))
      df_scope$StateGroup<- paste(scopeIn,"All",sep="|")
      # df_baseline represents the dataset against which site results will be compared and assigned a state score  
      df_baseline <- subset(df, tolower(SWQAltitude)=="all" &
                              tolower(SWQLanduse)=="all" &
                              tolower(Scope)=="nz" &
                              tolower(Measurement)==tolower(wqparam[i]))  
      # Region|Upland, Catchment|Upland, Site|Upland     Region|Lowland, Catchment|Lowland, Site|Lowland
    } else if(comparison==2){
      df_scope           <- subset(df, tolower(Scope)==tolower(scopeIn) &
                                     tolower(SWQAltitude)==tolower(altitude) &
                                     tolower(Measurement)==tolower(wqparam[i]))
      df_scope$StateGroup<- paste(scopeIn,altitude,sep="|")
      df_baseline <- subset(df, tolower(SWQAltitude)==tolower(altitude) &
                              tolower(SWQLanduse)=="all" &
                              tolower(Scope)=="nz" &
                              tolower(Measurement)==tolower(wqparam[i]))  
      ...
      df_scope           <- subset(df, tolower(Scope)==scopeIn &
                                     tolower(SWQLanduse)==LandcoverIn &
                                     tolower(Measurement)==tolower(wqparam[i]))
      df_scope$StateGroup<- paste(scopeIn,LandcoverIn,sep="|")
      df_baseline <- subset(df, tolower(SWQAltitude)=="all" &
                              tolower(SWQLanduse)==LandcoverIn &
                              tolower(Scope)=="nz" &
                              tolower(Measurement)==tolower(wqparam[i]))  
      ...
      df_scope           <- subset(df, tolower(Scope)==scopeIn &
                                     tolower(SWQAltitude)==altitude &
                                     tolower(SWQLanduse)==LandcoverIn &
                                     tolower(Measurement)==tolower(wqparam[i]))
      if(length(df_scope[,1])!=0){ # Testing for zero records returned - allows for zero Upland - Urban combination during testing.
        df_scope$StateGroup<- paste(scopeIn,altitude,LandcoverIn,sep="|")
        df_baseline <- subset(df, tolower(SWQAltitude)==altitude &
                                tolower(SWQLanduse)==LandcoverIn &
                                tolower(Scope)=="nz" &
                                tolower(Measurement)==tolower(wqparam[i]))
      } 
...
calcTrendScore.sig <- function(df1){
  #names(seasonalkendall) <- c("LawaSiteID","Measurement","Sen.Pct","Sen.Slope","p.value")
  #trendscores <- calcTrendScore(seasonalkendall)
  
  df1 <- na.omit(df1)
  
  for(i in 1:length(df1[,1])){
    if(df1$Measurement[i]=="BDISC"){
...
calcTrendScore.ci <- function(df1){
  ...
  df1$TrendScore[df1$Measurement=="BDISC"&df1$Sen.Slope<0] <- df1$TrendScore[df1$Measurement=="BDISC"&df1$Sen.Slope<0]*-1
  df1$TrendScore[df1$Measurement!="BDISC"&df1$Sen.Slope>0] <- df1$TrendScore[df1$Measurement!="BDISC"&df1$Sen.Slope>0]*-1
  return(df1)
...
PlotSites1 <- function(df,label){
  ...
  n <- 1
  plot = list()
  for(i in unique(df$LawaSiteID)){
    #    
    dfSite<-subset(df, df$LawaSiteID==i)
    print(i)
...
PlotSites <- function(df,label){
  ...
  plots <- dlply(df, "LawaSiteID", '%+%', el=p)
...
sk <- function(lawa_ids,freqNum,freqText,df,rate,years,months){
  k <- 1 # counter for sites/Measurements meeting minimum N
  for(i in 1:length(lawa_ids)){
    months<-freqNum
    l <- lawa_ids[i]
    df1 <- subset(df, LawaSiteID==l)
    Measurements <- as.character(unique(df1$Measurement))
    ...
    cat("length(Measurements)",length(Measurements),"\n")
    cat(Measurements,"\n")
    
    # calculating seasonal Kendall statistics for individual Measurements for an individual site
    for(j in 1:length(Measurements)){
      # exclude sites/Measurements where period of record is insufficient
      
      if(length(subset(df1,Measurement==Measurements[j])[,1])>=rate*(years*months)){
        ...
        m <-matrix(data=c(l,Measurements[j],s$sen.slope.pct,s$sen.slope,s$p.value),nrow=1,ncol=5,byrow=TRUE)
...
seaKenEPA <- function(x,stat="median"){
  ...
  sk.sen.slope     <- as.numeric(test.output$estimate[2])         # slope - estimated Measurement
...
#   
#   myData <-  x
#   myVar <-  as.character(myData$Measurement[1])
#   
...
samplesByInterval <- function (StartYear, EndYear, StartMonth, EndMonth, lawadata) {
  ...
  wcrc <- wcrc[complete.cases(wcrc$CouncilSiteID),]
...
leftCensored <-  function(x, forwardT="log", reverseT="exp") {     
  ...
  myData <- x
  #names(myData) <- c("CouncilSiteID","Date","converted_values","Method","Measurement","Censored","CenType","dflag"  )
  myData$dflag <- as.character(myData$dflag)
  #cat("Site = ", as.character(x$CouncilSiteID)[1], " & Variable = ", as.character(x$Measurement)[1], "\n")
  ...
  if(0 == sum("dl"==myData$dflag)) {   
    ...
    if(sum(data2$dflag == "dl") == 0 | nrow(data2) < 12) { # if there are no dl values or less than a year of myData
      ...
      #      print(as.character(myData$CouncilSiteID))
      #browser()
...
## survreg {survival}
## edited survreg function - customising to take data for censoring - dropping a few of the Measurement checks as
                         ...
                         x = FALSE, y = TRUE, robust = FALSE, score = FALSE, ...) 
{
  ...
  if (is.null(ptemp)) {
    if (!is.null(parms)) 
      stop(paste(dlist$name, "distribution has no optional Measurements"))
  } else {
    if (!is.numeric(ptemp)) 
      stop("Default Measurements must be a numeric vector")
    if (!missing(parms)) {
      temp <- unlist(parms)
      indx <- match(names(temp), names(ptemp))
      if (any(is.na(indx))) 
        stop("Invalid Measurement names")
