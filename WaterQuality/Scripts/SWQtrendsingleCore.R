...
try(dir.create(paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"))))
...
if(!exists('wqdata')){
  ...
  cat(wqdataFileName)
  wqdata=read.csv(wqdataFileName,stringsAsFactors = F)
  rm(wqdataFileName)
  wqdata$SWQLandcover[is.na(wqdata$SWQLandcover)]=riverSiteTable$SWQLandcover[match(wqdata$LawaSiteID[is.na(wqdata$SWQLandcover)],riverSiteTable$LawaSiteID)]
  ...
  wqdata$SWQLandcover[tolower(wqdata$SWQLandcover)%in%c("unstated","")] <- NA
  wqdata$SWQLandcover[tolower(wqdata$SWQLandcover)%in%c("forest","native","exotic","natural")] <- "Forest"
...
#10 year trend ####
wqdatafor10=wqdata%>%filter(Year>=startYear10 & Year <= EndYear & Measurement!="PH")
wqdatafor10$Season <- wqdatafor10$Month
SeasonString <- sort(unique(wqdatafor10$Season))

usites=unique(wqdatafor10$LawaSiteID)
uMeasures=unique(wqdatafor10$Measurement)
trendTable10=structure(list(LawaSiteID=NA,Measurement=NA,Observations = NA_integer_, KWstat = NA_real_,pvalue = NA_real_,
                            ...
                            prop.unique=NA_real_,no.censorlevels=NA_real_,Median = NA_real_, Sen_VarS = NA_real_, AnnualSenSlope = NA_real_,
                            Intercept = NA_real_, Lci = NA_real_, Uci = NA_real_,AnalysisNoteSS=NA_real_,Sen_Probability=NA_real_,
                            Probabilitymax = NA_real_, Probabilitymin = NA_real_, Percent.annual.change = NA_real_), class = "data.frame")
nMax=length(table(wqdatafor10$LawaSiteID,wqdatafor10$Measurement)[table(wqdatafor10$LawaSiteID,wqdatafor10$Measurement)>0])
    ...
    if(dim(subSubDat)[1]>0){
      ...
      if(numSamples >= 0.9*120 & numYears>=9){
        ...
        if(!is.na(st$pvalue)&&st$pvalue<0.05){
          ...
          newRow=cbind(LawaSiteID=usites[usite],Measurement=uMeasures[uparam],st,sk,sss)
        }else{
          ...
          newRow=cbind(LawaSiteID=usites[usite],Measurement=uMeasures[uparam],st,mk,ss)
        }
        suppressWarnings(rm(st,sk,sss,ss,mk))
...
trendTable10$Probability[trendTable10$Measurement!="BDISC"]=1-(trendTable10$Probability[trendTable10$Measurement!="BDISC"])
...
save(trendTable10,file=paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/Trend10Year.rData"))
save(passCriteria10,file=paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/PassCriteria10.rData"))
...
# if(any(tt10agency=="mdc" & trendTable10$Measurement=="DRP")){
#   trendTable10 <- trendTable10[-which(tt10agency=='mdc'& trendTable10$Measurement=="DRP"),]
#   # 3257 to 3231
#   tt10agency=riverSiteTable$Agency[match(trendTable10$LawaSiteID,riverSiteTable$LawaSiteID)]
# }
# if(any(tt10agency=="mdc" & trendTable10$Measurement=="ECOLI")){
#   trendTable10 <- trendTable10[-which(tt10agency=='mdc'& trendTable10$Measurement=="ECOLI"),]
#   #3231 to 3205
#   tt10agency=riverSiteTable$Agency[match(trendTable10$LawaSiteID,riverSiteTable$LawaSiteID)]
# }
...
#5 year trend ####
wqdatafor5=wqdata%>%filter(Year>=startYear5 & Year <= EndYear & Measurement!="PH")
wqdatafor5$Season=wqdatafor5$Month
usites=unique(wqdatafor5$LawaSiteID)
uMeasures=unique(wqdatafor5$Measurement)
trendTable5=structure(list(LawaSiteID=NA,Measurement=NA,
...
nMax=length(table(wqdatafor5$LawaSiteID,wqdatafor5$Measurement)[table(wqdatafor5$LawaSiteID,wqdatafor5$Measurement)>0])
  ...
  for(uparam in 1:length(uMeasures)){
    ...
    if(dim(subSubDat)[1]>0){
      ...
      if(numSamples >= 0.9*60){
        ...
        suppressWarnings(rm(sk,sss,ms,ss,st))
...
trendTable5$Probability[trendTable5$Measurement!="BDISC"]=1-(trendTable5$Probability[trendTable5$Measurement!="BDISC"])
save(trendTable5,file=paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/Trend5Year.rData"))
save(passCriteria5,file=paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/PassCriteria5.rData"))
...
wqdataforQ10=wqdata%>%filter(Year>=startYear10 & Year <= EndYear & Measurement!="PH")
...
uMeasures=unique(wqdataforQ10$Measurement)
trendTableQ10=structure(list(LawaSiteID=NA,Measurement=NA,
                            Observations = NA_integer_, KWstat = NA_real_,pvalue = NA_real_,Probability = NA_real_,
                             nObs = NA_integer_, S = NA_real_, VarS = NA_real_,D = NA_real_, tau = NA_real_, Z = NA_real_, p = NA_real_, 
                             Median = NA_real_, VarS = NA_real_, AnnualSenSlope = NA_real_, Intercept = NA_real_, Lci = NA_real_, Uci = NA_real_, TrendCategory = NA, 
                             TrendDirection = NA, Sen_Probability = NA_real_, Probabilitymax = NA_real_, Probabilitymin = NA_real_, Percent.annual.change = NA_real_), class = "data.frame")
nMax=length(table(wqdataforQ10$LawaSiteID,wqdataforQ10$Measurement)[table(wqdataforQ10$LawaSiteID,wqdataforQ10$Measurement)>0])
...
trendTableQ10$Probability[trendTableQ10$Measurement!="BDISC"]=1-(trendTableQ10$Probability[trendTableQ10$Measurement!="BDISC"])
save(trendTableQ10,file=paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/Trend10YearQ.rData"))
save(passCriteriaQ10,file=paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/PassCriteriaQ10.rData"))
...
load(tail(dir(ignore.case=T,
              path = "h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",
...
if(any(ttQ10agency=="mdc" & trendTableQ10$Measurement=="DRP")){
  trendTableQ10 <- trendTableQ10[-which(ttQ10agency=='mdc'& trendTableQ10$Measurement=="DRP"),]
  # 5399 to 5373
  ttQ10agency=riverSiteTable$Agency[match(trendTableQ10$LawaSiteID,riverSiteTable$LawaSiteID)]
}
if(any(ttQ10agency=="mdc" & trendTableQ10$Measurement=="ECOLI")){
  trendTableQ10 <- trendTableQ10[-which(ttQ10agency=='mdc'& trendTableQ10$Measurement=="ECOLI"),]
  #5373 to 7346
  ttQ10agency=riverSiteTable$Agency[match(trendTableQ10$LawaSiteID,riverSiteTable$LawaSiteID)]
}
...
# 104
combTrend=rbind(combTrend,trendTable5) #[which(!paste0(trendTable5$LawaSiteID,trendTable5$Measurement)%in%paste0(combTrend$LawaSiteID,combTrend$Measurement)),]
# 104 + 216 = 320
combTrend=rbind(combTrend,trendTableQ10[which(!paste0(trendTableQ10$LawaSiteID,trendTableQ10$Measurement)%in%paste0(combTrend$LawaSiteID,combTrend$Measurement)),])
# 320 + 0 = 320
...
combTrend$Landcover =   riverSiteTable$SWQLandcover[match(combTrend$LawaSiteID,riverSiteTable$LawaSiteID)]
...
MCItrend=read.csv(tail(dir(path="h:/ericg/16666LAWA/LAWA2019/MacroInvertebrates/Analysis",
                           pattern='MacroMCI_Trend',full.names = T,recursive = T,ignore.case = T),1),stringsAsFactors = F)
IDs=read.csv(tail(dir('h:/ericg/16666LAWA/LAWA2019/','RiverAndMacroSiteLocns',recursive = F,full.names = T),1),stringsAsFactors = F)
MCItrend$Site=IDs$Site[match(MCItrend$LawaSiteID,IDs$LawaSiteID)]
MCItrend$Landcover=NA
...
combTrendExport <- combTrend%>%select(LawaSiteID,Site,Measurement,Altitude,Landcover,TrendScore,Region,Frequency,period)
combTrendExport <- rbind(combTrendExport,MCItrend%>%select(LawaSiteID,Site,Measurement,Altitude,Landcover,TrendScore,Region,Frequency,period))

write.csv(combTrendExport%>%select(-Site)%>%filter(Measurement!="MCI"),
          paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/RiverWQ_Trend_ForITE",
                         format(Sys.time(),"%Hh%Mm-%d%b%Y"),".csv"),row.names = F)
write.csv(combTrendExport,paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",
                                 format(Sys.Date(),"%Y-%m-%d"),"/RiverWQAndMCI_Trend_ForPublicDownload",
                                                 format(Sys.time(),"%Hh%Mm-%d%b%Y"),".csv"),row.names = F)
rm(combTrendExport)
# combTrendExport=read.csv(tail(dir("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/","RiverWQ_Trend_ForITE",recursive = T,full.names = T),1),stringsAsFactors = F)



#RiverAwards sub analysis??
wqdata$Season=wqdata$Month
savePlott=T
usites=unique(combTrend$LawaSiteID)
uMeasures=unique(combTrend$Measurement)%>%as.character
for(uparam in seq_along(uMeasures)){
  ...
  if(savePlott){
    tiff(paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/BestWorst",uMeasures[uparam],".tif"),
...
#   load("H:/ericg/16666LAWA/LAWA2019/MacroInvertebrates/4.Analysis/2018-09-12/Trend10Year.rData")
...
MCItrend=read.csv(tail(dir(path="h:/ericg/16666LAWA/LAWA2019/MacroInvertebrates/Analysis",
                            pattern='MacroMCI_Trend',full.names = T,recursive = T,ignore.case = T),1),stringsAsFactors = F)
TrendsForPlotting = rbind(combTrend%>%dplyr::filter(standard%in%c('gold','bronze'))%>%dplyr::select(LawaSiteID,Measurement,Region,TrendScore),
                          MCItrend%>%dplyr::select(LawaSiteID,Measurement,Region,TrendScore))
rm(MCItrend)

table(TrendsForPlotting$Measurement[which(TrendsForPlotting$TrendScore==-99)]) ->naTab
...
TrendsForPlotting$Measurement <- factor(TrendsForPlotting$Measurement,levels=c("BDISC","TURB","TN","TON","NH4","TP","DRP","ECOLI","MCI"))
...
tiff(paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/TenYearMonthlyAndQuarterlyTrendsNumbres.tif"),
...
tiff(paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/TenYearMonthlyAndQuarterlyTrendsPercentage.tif"),
     width = 8,height=8,units='in',res=300,compression='lzw',type='cairo')
par(mfrow=c(1,1),mar=c(5,10,6,2))
barplot(tbp,main="Ten year trends, monthly and quarterly",las=2,
        col=c("#dd1111FF","#ee4411FF","#bbbbbbFF","#11cc11FF","#008800FF"),yaxt='n',xaxt='n') #"#dddddd",
...
# t10q <- plot(factor(trendTableQ10$Measurement),trendTableQ10$ConfCat,col=c("#dd1111FF","#ee4411FF","#bbbbbbFF","#11cc11FF","#008800FF"),main="10 Year quarterly")
# write.csv(t5,paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/Trend5Year.csv"))
# write.csv(t10,paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/Trend10Year.csv"))
# write.csv(t10q,paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/Trend10YearQuarterly.csv"))
...
# colMPs=-0.5+(1:8)*1.2
# 
# tiff(paste0("h:/ericg/16666LAWA/LAWA2019/WaterQuality/4.Analysis/",format(Sys.Date(),"%Y-%m-%d"),"/TrendByPeriod.tif"),
